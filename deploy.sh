...
if [ -f "$SCRIPT_DIR/versions.env" ]; then
    ...
    echo "BACKEND_VERSION=1.1.1"  > "$SCRIPT_DIR/versions.env"
    echo "FRONTEND_VERSION=1.1.1" >> "$SCRIPT_DIR/versions.env"
    source "$SCRIPT_DIR/versions.env"
fi
...
show_help() {
    ...
    echo ""
    echo "Actualización:"
    echo "  1. Editar versions.env  (ej: BACKEND_VERSION=1.1.1)"
