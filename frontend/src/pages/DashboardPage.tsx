...
const KPICard = ({
  label, value, sub, color, icon, theme,
}: {
  label: string; value: number | string; sub?: string; color: string; icon: ReactNode;
  theme: any;
}) => (
  <div style={{
    ...
    <div>
      <div style={{ fontSize: 28, fontWeight: 700, color: theme.textPrimary, lineHeight: 1.1 }}>{value}</div>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([clientsApi.getAll(), calendarApi.getProximos(), feesApi.resumen(), notificationsApi.unreadCount()])
      ...
      .catch(() => {
        setKpis({ clientesActivos: 0, clientesTotal: 0, vencimientosProximos: 0, vencimientosVencidos: 0, honorariosPendientes: 0 });
        setProximos([]); setUnread(0);
      })
    ...
    <div style={{ padding: '28px 32px', maxWidth: 1100 }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: theme.textSecondary, fontSize: 14 }}>
          ...
          {unread > 0 && (
            <span style={{ marginLeft: 12, padding: '2px 10px', borderRadius: 20, background: '#FEF3C7', color: '#D97706', fontSize: 12, fontWeight: 500 }}>
      ...
      {loading ? (
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
            <KPICard theme={theme} label="Clientes activos" value={kpis?.clientesActivos ?? 0} sub={`de ${kpis?.clientesTotal ?? 0} en total`} color={theme.accent}
            ...
            />
            <KPICard theme={theme} label="Vencimientos próximos" value={kpis?.vencimientosProximos ?? 0} sub="pendientes o por vencer" color="#0EA5E9"
            ...
            />
            <KPICard theme={theme} label="Vencimientos vencidos" value={kpis?.vencimientosVencidos ?? 0} sub="requieren atención" color="#DC2626"
            ...
            />
            <KPICard theme={theme} label="Honorarios pendientes" value={kpis?.honorariosPendientes ?? 0} sub="sin cobrar" color="#D97706"
          ...
          <div style={{ background: theme.cardBg, borderRadius: 12, boxShadow: theme.cardShadow, overflow: 'hidden', border: `1px solid ${theme.cardBorder}` }}>
            <div style={{ padding: '16px 20px', borderBottom: `1px solid ${theme.cardBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ fontSize: 15, fontWeight: 600, color: theme.textPrimary }}>Vencimientos próximos</h2>
              <button onClick={() => navigate('/calendar')} style={{ background: 'none', border: 'none', color: theme.accentText, fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>
                Ver todos →
              </button>
            </div>
            {proximos.length === 0 ? (
              <div style={{ padding: 32, textAlign: 'center', color: theme.textMuted, fontSize: 14 }}>No hay vencimientos próximos</div>
            ) : (
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: theme.tableHeaderBg }}>
                    {['Cliente', 'Obligación', 'Vence', 'Estado'].map((h) => (
                      <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: theme.textMuted, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {proximos.map((v: any) => (
                    <tr
                      ...
                      style={{ borderTop: `1px solid ${theme.tableBorder}`, cursor: 'pointer', transition: 'background 0.1s' }}
                    ...
                    >
                      <td style={{ padding: '12px 16px', fontSize: 14, color: theme.textPrimary, fontWeight: 500 }}>{v.clienteNombre ?? v.client?.nombre ?? '—'}</td>
                      <td style={{ padding: '12px 16px', fontSize: 13, color: theme.textSecondary }}>{v.tipo?.replaceAll('_', ' ').toUpperCase()}</td>
                      <td style={{ padding: '12px 16px', fontSize: 13, color: theme.textPrimary }}>{new Date(v.fechaVencimiento).toLocaleDateString('es-UY')}</td>
